...
public class AgingBarrelCategory implements DisplayCategory<AgingBarrelDisplay> {

    @Override
    public CategoryIdentifier<AgingBarrelDisplay> getCategoryIdentifier() {
        return AgingBarrelDisplay.FERMENTATION_BARREL_DISPLAY;
    }
    ...
    public Renderer getIcon() {
        return EntryStacks.of(NetherObjectRegistry.WARPED_FERMENTATION_BARREL.get());
    }

    @Override
    public List<Widget> setupDisplay(AgingBarrelDisplay display, Rectangle bounds) {
        Point startPoint = new Point(bounds.getCenterX() - 55, bounds.getCenterY() - 13);
        List<Widget> widgets = Lists.newArrayList();
        widgets.add(Widgets.createRecipeBase(bounds));
        widgets.add(Widgets.createArrow(new Point(startPoint.x + 54, startPoint.y - 1)).animationDurationTicks(AgingBarrelBlockEntity.COOKING_TIME_IN_TICKS));
